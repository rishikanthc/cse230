...
available_at(X,Y) :- taqueria(Y,_,R), isin(X,R).

multi_available(X) :- available_at(X,Z), available_at(X,Y), not(Z=Y). 

overworked(X) :- taqueria(A,E1,_), isin(X, E1), taqueria(B,E2,_), isin(X,E2), not(A=B). 
